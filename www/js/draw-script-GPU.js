...
function StartBrush(path, color) {
    ...
    return new Promise((resolve) => {
        const key = `${path}|${color}|${scaleTexture}`;
        const cached = brushCache.find(item => item.key === key);
        if (cached) {
        ...
        myImageObj.crossOrigin = 'anonymous';
        myImageObj.src = path;

        myImageObj.onload = () => {
            const w = Math.max(1, Math.round(myImageObj.width * scaleTexture));
            const h = Math.max(1, Math.round(myImageObj.height * scaleTexture));
            const tempCanvas = document.createElement('canvas');
            tempCanvas.width = w;
            tempCanvas.height = h;

            // Tenta usar WebGL para renderizar a textura colorizada
            const gl = tempCanvas.getContext('webgl') || tempCanvas.getContext('experimental-webgl');
            if (!gl) {
                // fallback para 2D caso WebGL não esteja disponível
                const tempCtx = tempCanvas.getContext('2d');
                tempCtx.drawImage(myImageObj, 0, 0, w, h);
                tempCtx.fillStyle = color;
                tempCtx.globalCompositeOperation = 'source-in';
                tempCtx.fillRect(0, 0, w, h);
                tempCtx.globalCompositeOperation = 'source-over';
                brushCache.push({ key: key, path: path, canvas: tempCanvas });
                currentBrush = tempCanvas;
                resolve(currentBrush);
                return;
            }

            // utilitários para shaders
            ...
            const fragmentSrc = `precision mediump float; uniform sampler2D u_image; uniform vec4 u_color; varying vec2 v_texCoord; void main(){ vec4 tex = texture2D(u_image, v_texCoord); float a = tex.a; vec3 rgb = u_color.rgb * a; gl_FragColor = vec4(rgb, a * u_color.a); }`;
            ...
            gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, myImageObj);
            ...
            brushCache.push({ key: key, path: path, canvas: tempCanvas, color: color });
            currentBrush = tempCanvas;
            resolve(currentBrush);
        };
    });
...
function strokenize(ctx,shape){
    ...
    if (points.length >= 2) { // precisa de pelo menos x,y
        for (let i = 2; i < points.length; i += 2) { // pula de 2 em 2
            coisanoTextura(points,i,ctx,shape);
        }
    }
}
...
function coisanoTextura(points, i, ctx,shape) {
